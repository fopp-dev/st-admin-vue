      ...
      <div class="table">
        ...
        >
          <!-- <el-table-column type="expand"> -->
            ...
            <!-- </template> -->
          <!-- </el-table-column> -->
          <el-table-column width="120px" prop="userId" label="用户ID">
          ...
          <el-table-column width="120px" prop="realName" label="用户名">
            <template slot-scope="scope">
              <p>
                {{ scope.row.realName }}
          ...
          <el-table-column prop="regTime" width="130px" label="注册时间">
            <template slot-scope="scope">
              <p>
                {{ scope.row.regTime }}/<span class="small">{{
                  scope.row.id
                }}</span>
          ...
          <el-table-column prop="agentName" width="120px" label="代理">
          </el-table-column>
          <el-table-column prop="inMoney" label="入金">
            <template slot-scope="scope">
              <p class="number proColor bounceIn">
                {{
                  (
                    Number(scope.row.inMoney)
                  ).toFixed(2)
                }}
              </p>
            </template>
          </el-table-column>
          <el-table-column width="110px" prop="outMoney" label="出金">
            <template slot-scope="scope">
              <p class="number proColor bounceIn">
                {{
                  (
                    Number(scope.row.outMoney )
                  ).toFixed(2)
                }}
              </p>
            </template>
          </el-table-column>
          <el-table-column width="110px" prop="readyInMoney" label="充值待审核">
            <template slot-scope="scope">
              <p class="number proColor">
                {{ scope.row.readyInMoney }}
              </p>
            </template>
          </el-table-column>
          <el-table-column width="110px" prop="readyOutMoney" label="提现待审核">
            <template slot-scope="scope">
              <p class="number proColor bounceIn">
                {{ scope.row.readyOutMoney }}
              </p>
            </template>
          </el-table-column>
          <el-table-column width="110px" prop="totalAbuy" label="A股总持仓">
            <template slot-scope="scope">
              <p class="number proColor">
                {{ scope.row.totalAbuy }}
              </p>
          ...
          <el-table-column width="110px" prop="totalHbuy" label="港股总持仓">
            <template slot-scope="scope">
              <p class="number proColor bounceIn">
                {{ scope.row.totalHbuy }}
              </p>
          ...
          <el-table-column width="110px" prop="userCapital" label="总资金">
            <template slot-scope="scope">
              <p class="number proColor">
                {{ scope.row.userCapital }}
              </p>
            </template>
          </el-table-column>
          <el-table-column width="110px" prop="userAmt" label="A股总资产">
            <template slot-scope="scope">
              <p class="number proColor">
                {{ scope.row.userAmt }}
              </p>
          ...
          <el-table-column width="110px" prop="userHmt" label="港股资金">
            <template slot-scope="scope">
              <p class="number proColor">
                {{ scope.row.userHmt }}
              </p>
...
export default {
  ...
  methods: {
    ...
    async refreshList() {
      ...
      let data = await api.getUserAbstractList(opts);
      this.refresh = false;
      if (data.status === 0) {
    ...
    onExport() {
      require.ensure([], () => {
        ...
        const filterVal = [
          ...
          "feeA", // A股手续费
          "feeH", // 港股手续费
          "totalUserCapital", // 总资金
          "userStockACapital", // A股本金
          "userStockHKCapital", // 港股本金
          "allAProLose",
          "allHProLose",
        ];
        ...
        export_json_to_excel(tHeader, data, "摘要列表" + year + month + day);
      });
    },
    async getAgentList() {
      // 获取下级代理数据
