<template>
  <div>
    ...
    <el-popover
      ...
      trigger="hover">
      <div>
        <p>保证金 = 买入手数 * 每手保证金 * 汇率</p>
        <p>买涨的情况下：浮动盈亏 =（ 卖出点数 - 买入点数 ） * 数量 * 标准手 * 汇率</p>
        <p>买跌的情况下：浮动盈亏 = -（ 卖出点数 - 买入点数 ） * 数量 * 标准手 * 汇率</p>
        <p>总盈亏 = （浮动盈亏 - 手续费） * 汇率</p>
      </div>
    </el-popover>
    <div class="table">
      <el-table
          ...
          <template slot-scope="props">
            <div class="rule">
              <p v-if="props.row.orderDirection == '买涨'">
                ...
                {{props.row.futuresStandard}} = {{((props.row.sellOrderPrice -
                props.row.buyOrderPrice)*props.row.orderNum*props.row.futuresStandard).toFixed(2)}}
                {{props.row.coinCode}} ≈ {{props.row.profitAndLose * props.row.sellRate}} CNY
              </p>
              <p v-else>
                浮动盈亏：-( {{props.row.sellOrderPrice}} - {{props.row.buyOrderPrice}} ) * {{props.row.orderNum}} *
                {{props.row.futuresStandard}} = {{(-(props.row.sellOrderPrice -
              ...
              <p>
                ...
                props.row.orderFee}} {{props.row.coinCode}} ≈ {{props.row.allProfitAndLose * props.row.sellRate}} CNY
              </p>
            </div>
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="手续费">
                <span>{{ props.row.orderFee}} {{props.row.coinCode}} ≈ {{ (props.row.orderFee * props.row.sellRate).toFixed(2)}} CNY</span>
              </el-form-item>
              <el-form-item label="数量/手">
                <span>{{ props.row.orderNum}}</span>
              </el-form-item>
              <el-form-item label="卖出汇率">
                <span>1{{props.row.coinCode}} ≈ {{ props.row.sellRate}}CNY</span>
              </el-form-item>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <p class="bounceIn">
              <span
                :class="scope.row.profitAndLose<0?'green':'red'">{{scope.row.profitAndLose}} {{scope.row.coinCode}}</span>
            </p>
            <p>≈{{(scope.row.profitAndLose * scope.row.sellRate).toFixed(2)}} CNY
            </p>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            <p class="bounceIn">
              <span :class="scope.row.allProfitAndLose<0?'green':'red'">{{scope.row.allProfitAndLose}} {{scope.row.coinCode}}</span>
            </p>
            <p>≈{{(scope.row.allProfitAndLose * scope.row.sellRate).toFixed(2)}} CNY
            </p>
          </template>
        </el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="danger" plain size="small" @click="toDelete(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="page-box">
...
export default {
  ...
  methods: {
    ...
    toDelete (row) {
      this.$confirm('确认删除持仓单？此操作不可恢复', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        let data = await api.delPositionSellGg({ positionId: row.id })
        if (data.status === 0) {
