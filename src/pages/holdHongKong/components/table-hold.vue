      ...
      >
        ...
        <el-table-column width="70px" prop="positionType" label="账户类型">
          <template slot-scope="scope">
            <el-tag :type="scope.row.positionType == 1 ? 'info' : 'success'"
        ...
        <el-table-column prop="positionSn" label="持仓订单号/id">
          <template slot-scope="scope">
            ...
            <a
              class="hide-td"
              href="javascript:;"
              :title="scope.row.positionSn"
              >{{ scope.row.positionSn }}</a
            >
          </template>
        </el-table-column>
        ...
        <el-table-column prop="stockName" label="股票名称">
          <template slot-scope="scope">
            <p>
              <span
                :class="
                  scope.row.stockPlate == '科创' ? 'kc-mark' : 'kc-mark a-mark'
                "
                >{{ scope.row.stockPlate == "科创" ? "科创" : "A股" }}</span
              >
            </p>
            <p>
              {{ scope.row.stockName }}
          ...
          <template slot-scope="scope">
            <p>
              ...
              <i
                ...
                class="red iconfont icon-up"
              ></i>
              <i
                v-if="scope.row.orderDirection == '买跌'"
                class="green iconfont icon-down"
              ></i>
            </p>
          </template>
        </el-table-column>
        <el-table-column prop="buyOrderPrice" label="买入价"> </el-table-column>
        <el-table-column prop="now_price" label="现价">
          <template slot-scope="scope">
            <div
              v-if="scope.row.now_price"
              :class="
                changeTextClass[scope.$index] == true
                  ? 'heartBeat  tab-number'
                  : ' tab-number'
              "
            >
              <p
        ...
        <el-table-column prop="profitAndLose" label="浮动盈亏">
          <template slot-scope="scope">
            <div class="bounceIn tab-number">
              <p
                :class="
                  changeTextClass[scope.$index] == true ? 'heartBeat' : ''
                "
              >
       ...
       <el-table-column
        ...
        -->
        <el-table-column fixed="right" prop="isLock" width="180px" label="操作" v-if="$store.state.showMenu == 1">
          <template slot-scope="scope">
            <el-button
            ...
            <el-button
              ...
              plain
              size="small"
              @click="positionUnLock(scope.row)"
              >解锁
            </el-button>
...
export default {
  ...
  methods: {
    ...
    async refreshList() {
      ...
      let data = await api.OrderlistGg(opts);
      this.refresh = false;
      if (data.status === 0) {
    ...
    onExport() {
       require.ensure([], () => {
        ...
        const tHeader = ['持仓订单号', '用户名', '股票名称', '方向','买入价', '现价', '浮动盈亏', '总盈亏']
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['positionSn', 'nickName', 'stockName', 'orderDirection', 'buyOrderPrice', 'now_price', 'profitAndLose','allProfitAndLose']
        ...
        export_json_to_excel(tHeader, data, '融资持仓单' + year + month + day)
      })
    },
    async getAgentList() {
      // 获取下级代理数据
    ...
    toSell(row) {
      ...
      this.$confirm("确认要强制平仓?", "提示", {
        ...
        .then(async () => {
          let data = await api.positionSellGg({ positionSn: row.positionSn });
          if (data.status === 0) {
    ...
    positionLock(row) {
      ...
      this.$prompt("请输入锁定原因?", "提示", {
        ...
        .then(async ({ value }) => {
          ...
          let data = await api.positionLockGg(opts);
          if (data.status === 0) {
            this.$message.success(data.msg);
            this.getList();
          } else {
