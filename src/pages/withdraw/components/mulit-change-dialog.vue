      ...
      <div>
        <el-form :inline="true" :model="form" ref="ruleForm" label-width="80px" :rules="rule" class="demo-form-inline">
          <el-form-item label="订单ID" prop="chargeId">
              <div>
            <label v-for="(item,index) in info" :key="index">{{item.id}} <span v-if="index!=info.length-1">，</span></label>
              </div>
          ...
          <el-alert
            ...
            title='状态修改为"提现成功",表示转账成功,用户的资金会减少'
            type="error"
            close-text="知道了">
          </el-alert>
          <el-alert
  ...
  methods: {
    submit (formName) {
      ...
      this.$refs[formName].validate(async (valid) => {
        ...
        withIds =  withIds.substring(0, withIds.lastIndexOf(','));
        if (valid) {
          let opts = {
            withIds: withIds,
          ...
          let data = await api.batchWithdrawUpdateState(opts)
          if (data.status === 0) {
