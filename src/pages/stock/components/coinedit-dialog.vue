    ...
    >
      <div>
        <el-form :inline="true" :model="form" ref="ruleForm" size="mini" label-width="150px" :rules="rule">
          ...
          <el-form-item label="转入汇差" prop="inDiff">
            <el-input v-model="form.inDiff" placeholder="转入汇差"></el-input>
          </el-form-item>
          <el-form-item label="转出汇差" prop="outDiff">
            <el-input v-model="form.outDiff" placeholder="转出汇差"></el-input>
          </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rule: {
        ...
        inDiff: [
          { required: true, message: '请输入转入汇差', trigger: 'blur' }
        ],
        outDiff: [
          { required: true, message: '请输入转出汇差', trigger: 'blur' }
        ],
        dynamicRate: [
  ...
  watch: {
    info (val) {
      if (val) {
        this.form.code = this.info.coinCode
        this.form.name = this.info.coinName
        this.form.defaultRate = this.info.defaultRate
        this.form.dynamicRate = this.info.dynamicRate
        this.form.isUse = this.info.isUse
        this.form.inDiff = this.info.inDiff
        this.form.outDiff = this.info.outDiff
      }
    }
  },
  computed: {},
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          let opts = {
            ...
            inDiff: this.form.inDiff,
            outDiff: this.form.outDiff,
          }
          let data = await api.editCoin(opts)
          if (data.status == 0) {
